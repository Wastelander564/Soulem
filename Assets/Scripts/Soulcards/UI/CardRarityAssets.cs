using UnityEngine;

[CreateAssetMenu(fileName = "CardRarityAssets", menuName = "Cards/Rarity Assets")]
public class CardRarityAssets : ScriptableObject
{
    public Sprite commonSprite;
    public Sprite rareSprite;
    public Sprite veryRareSprite;
    public Sprite epicSprite;
    public Sprite mythicalSprite;

    public Sprite GetSpriteForRarity(Rarity rarity)
    {
        return rarity switch
        {
            Rarity.Common => commonSprite,
            Rarity.Rare => rareSprite,
            Rarity.VeryRare => veryRareSprite,
            Rarity.Epic => epicSprite,
            Rarity.Mythical => mythicalSprite,
            _ => null,
        };
    }

    public Color GetColorForRarity(Rarity rarity)
    {
        return rarity switch
        {
            Rarity.Common => new Color(187f/255f, 187f/255f, 187f/255f),       // light gray
            Rarity.Rare => new Color(125f/255f, 191f/255f, 0f),               // greenish
            Rarity.VeryRare => new Color(65f/255f, 65f/255f, 255f/255f),      // blue
            Rarity.Epic => new Color(146f/255f, 65f/255f, 243f/255f),         // purple
            Rarity.Mythical => new Color(235f/255f, 211f/255f, 32f/255f),     // yellow
            _ => Color.white,
        };
    }
}
